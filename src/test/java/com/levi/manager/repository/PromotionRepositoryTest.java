...
public class PromotionRepositoryTest {
    ...
    private final String COMBO_NAME = "First Promotion Name";
    private final Double COMBO_PRICE = 3.0;
    private final FoodCategory COMBO_FOOD_CATEGORY = FoodCategory.CONVENIENCE;

    private final String SECOND_COMBO_NAME = "Second Promotion Name";
    ...
    public void findByRestaurantId() {
        repository.save(givenFirstPromotion());
        repository.save(givenSecondPromotion());
        List<Promotion> promotionsByRestaurant = repository.findByRestaurantId(RESTAURANT_ID);
        Assert.assertEquals(promotionsByRestaurant.get(COMBO).getId(), RESTAURANT_ID);
    }

    private Promotion givenFirstPromotion() {
        Promotion promotion = new Promotion();
        Restaurant restaurant = givenFirstRestaurant();
        restaurantRepository.save(restaurant);
        promotion.setRestaurant(restaurant);
        promotion.setName(COMBO_NAME);
        promotion.setPrice(COMBO_PRICE);
        promotion.setCategory(COMBO_FOOD_CATEGORY);
        return promotion;
    }
    ...
    private Promotion givenSecondPromotion() {
        ...
        restaurantRepository.save(restaurant);
        promotion.setRestaurant(restaurant);
        promotion.setName(SECOND_COMBO_NAME);
        promotion.setPrice(SECOND_COMBO_PRICE);
        promotion.setCategory(SECOND_COMBO_FOOD_CATEGORY);
        return promotion;
    }
